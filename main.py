...
from attack_graph.vulnerability_assessment import analyze_vulnerabilities_and_paths
...
def run_scan_pipeline(target):
    ...
    # Analyze vulnerabilities and critical paths
    analysis = analyze_vulnerabilities_and_paths(scan, G)

    # Format for Vis.js (optional, keep for dashboard)
    graph_data = {
        "nodes": [{"id": n, **d} for n, d in G.nodes(data=True)],
        "edges": [{"from": u, "to": v} for u, v in G.edges()]
    }

    # Return both the graph and the vulnerability analysis
    return {
        "graph": graph_data,
